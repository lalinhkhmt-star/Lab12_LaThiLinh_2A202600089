...
PASS_ICON = "✅" if UNICODE_OK else "[OK]"
FAIL_ICON = "❌" if UNICODE_OK else "[X]"
FILE_ICON = "📁" if UNICODE_OK else "[Files]"
LOCK_ICON = "🔒" if UNICODE_OK else "[Security]"
GLOBE_ICON = "🌐" if UNICODE_OK else "[API]"
DOCKER_ICON = "🐳" if UNICODE_OK else "[Docker]"
READY_ICON = "🎉" if UNICODE_OK else "[READY]"
WARN_ICON = "⚠️" if UNICODE_OK else "[WARN]"
...
def run_checks():
    ...
    print(f"\n{GLOBE_ICON} API Endpoints (code check)")
    ...
    if pct == 100:
        print(f"  {READY_ICON} PRODUCTION READY! Deploy nào!")
    elif pct >= 80:
        print(f"  {PASS_ICON} Almost there! Fix the {FAIL_ICON} items above.")
    elif pct >= 60:
        print(f"  {WARN_ICON} Good progress. Several items need attention.")
    else:
        print(f"  {FAIL_ICON} Not ready. Review the checklist carefully.")
